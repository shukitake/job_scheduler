...
if __name__ == "__main__":
    ...
    plt.ylabel("Time [s]")
    plt.savefig("./model/result/model_1/time.pdf", format="pdf")
    # resultに計算時間を保存する
    with open("./model/result/model_1/time.csv", "w") as f:
        for i in num_set:
            f.write(str(i) + "," + str(time_list[i - 3]) + "\n")
    # resultにジョブの順序を保存する
    with open("./model/result/model_1/job_order.csv", "w") as f:
        for i in num_set:
